...
st.set_page_config(
    page_title="八杉式・思考タイプ診断", page_icon="🧠", layout="centered"
)
...
.stButton>button { background-color: #deff9a; color: #000; font-weight: bold; border-radius: 8px; border: none; width: 100%; }
...
if not api_key:
    st.info(
        "👈 左側のサイドバーに「Gemini API Key」を入力すると、診断を開始できます。"
    )
else:
    # --- Stage 1 ---
    if st.session_state.stage == 1:
        ...
        ans1 = st.text_area(
            "あなたの決断・行動を入力してください（ご自身の言葉で記述）:",
            height=150,
        )
        if st.button("回答を送信（Gemini思考エンジン起動）"):
            if len(ans1.strip()) < 10:
                st.warning(
                    "思考の軌跡を判定するため、もう少し詳しく記述してください。"
                )
            else:
    ...
    # --- Stage 2 ---
        ...
        st.error(
            "⚠️ 警告：受検者の思考水準を検知。追加ストレッサー（理不尽な制約）が投入されました。"
        )
                ...
                with st.spinner("Geminiが回答の行間・感情・本性を深層解析中..."):
                    try:
                        res = evaluate_with_gemini(
                            api_key, st.session_state.history
                        )
                        ...
                        st.error(
                            f"解析エラーが発生しました。APIキーを確認してください: {e}"
                        )
    ...
    # --- Stage 3 ---
        ...
        st.success(
            "🎉 Geminiプロファイリング完了！分析レポートが生成されました。"
        )
        st.divider()
